...
var encode = function (json) {
  var features = json.features.map(feature => {
    ...
    if (type === 'LineString') {
      coordinatesString = encodeOffsets(coordinates, encodeOffsets)
    } else if (type === 'Polygon') {
      coordinates.forEach((coordinate, i) => {
        coordinatesString[i] = encodePolygon(coordinate, encodeOffsets[i] = [])
      })
    } else if (type === 'MultiPolygon') {
      coordinates.forEach((ploygon, i) => {
        encodeOffsets[i] = []
        coordinatesString[i] = []
        ploygon.forEach((coordinate, j) => {
          coordinatesString[i][j] = encodePolygon(coordinate, encodeOffsets[i][j] = [])
        })
      })
    } else {
